...
def main():
    ...
    save_dir_replaybuffer = os.path.join(args.save_dir,'replay_buffer', experiment_name)

    try:
        os.makedirs(save_dir_model)
        os.makedirs(save_dir_config)
        os.makedirs(save_dir_replaybuffer)
    except OSError as e:
        logger.error(e)
        exit()
    ...
    h5_file=h5py.File(save_dir_replaybuffer+'/mytest.hdf5','a')
    ...
    if checkpoint is not None:
        load_modules(agent.optimizer, agent.actor, checkpoint)
    ...
    agent_update_thread = threading.Thread(
        target=update_agent, name='agent_update', args=(agent, config))
