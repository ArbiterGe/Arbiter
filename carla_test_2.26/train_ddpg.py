...
from ddpg_carla import DDPGCarla
from  env_test import CarEnv
#import subprocess
    ...
    for i_episode in range(config.num_updates):
      ...
      #  agent.update()     
        ...
        for episode_timesteps in range(config.num_steps):
            # Sample actions           
            action = agent.get_action(obs, config.act_noise)
            ...
            #writer.add_scalar('carla_reward',reward,episode_timesteps)         
...
def explore_before_train(env, agent):
